...
/* end on off panel */
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h4 class="modal-title">Add Auction Product</h4>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>

        <!-- Modal body -->
        <div class="modal-body product-modal-open-height-scroll">
          <form action="{{ route('admin.auction.product-details-add') }}" id="product-admin-modal-id" method="POST" enctype="multipart/form-data">
          @csrf
            ...
            <div class="form-group">
              <label for="product-auction-time-id">Product Auction Time: ( in hrs )</label>
              <input type="number" class="form-control" placeholder="Enter Product Auction Timing" name="product_auction_timing" id="product-auction-time-id" required> 
            </div>
  ...
  <div class="modal" id="product-edit-show-modal">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title">Edit Auction Product</h4>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>

        <!-- Modal body -->
        <div class="modal-body product-modal-open-height-scroll">
          <form action="{{ route('admin.auction.product-details-update') }}" id="product-admin-modal-id" method="POST" enctype="multipart/form-data">
          @csrf
          <input type="hidden" name="edit_product_name_first_id" id="edit_product_name_first_id" value="" />
            ...
            <div class="form-group">
              <label for="product-auction-time-id">Product Auction Time: ( in hrs )</label>
              <input type="number" class="form-control" placeholder="Enter Product Auction Timing" name="product_auction_timing" id="edit-product-auction-time-id" required> 
            </div>
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>Auction Product Details</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
            <li class="breadcrumb-item active">Auction Product Details</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Auction Product Details</h3>
                <div class="row">
              ...
              <div class="card-tools">
                <div class="input-group input-group-sm" style="width: 150px;">
                  <a href="javascript: ;" class="btn btn-info btn-sm" id="add-banner-id" onclick="add_product_category()">Add Auction Product</a>
                </div>
              </div>
            </div>
            <!-- /.card-header -->
...
function edit_action(id)
{
  $("#product-edit-show-modal").modal('show');
    $.ajax({
        url: "{{ route('admin.auction.product-details-edit') }}",
        type: "GET",
        data: {id: id},
        dataType: "json",
        success: function(event){
          ...
          $("#edit-product-auction-time-id").val(event.product_auction_time);
...
{
    $.ajax({
        url: "{{ route('admin.auction.product-details') }}",
        type: "GET",
        dataType: "json",
        success: function(event){
...
{
    $.ajax({
      url: "{{ route('admin.auction.product-change-action') }}",
      type: "GET",
...
function del_action(id)
{
  ...
  {
    $.ajax({
      url: "{{ route('admin.auction.product-details-del') }}",
      type: "GET",
      data: {id: id},
      dataType: 'json',
      success: function(event){
